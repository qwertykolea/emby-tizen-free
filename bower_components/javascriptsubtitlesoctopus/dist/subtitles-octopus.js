define(["exports"], function (_exports) {
  ...
  var SubtitlesOctopus = function (options) {
    ...
    try {
      if (options.supportsWebAssembly !== false && typeof WebAssembly === "object" && typeof WebAssembly.instantiate === "function") {
    ...
    self.detectAlphaBug = options.detectAlphaBug !== false;
    (function () {
    ...
    function getSavedAlphaBugResult() {
      try {
        return localStorage.getItem('canvas_hasalphabug');
      } catch (err) {
        console.log('error reading from storage');
      }
      return null;
    }
    function setSavedAlphaBugResult(result) {
      try {
        localStorage.setItem('canvas_hasalphabug', result);
      } catch (err) {
    ...
    self.createCanvas = function () {
      if (!self.canvas) {
        ...
        self.canvasParent = self.canvas.parentNode;
      }
      self.ctx = self.canvas.getContext('2d');
      self.bufferCanvas = document.createElement('canvas');
      self.bufferCanvasCtx = self.bufferCanvas.getContext('2d');
      if (!self.detectAlphaBug) {
        return;
      }
      var savedHasAlphaBug = getSavedAlphaBugResult();
      if (savedHasAlphaBug === 'true') {
        self.hasAlphaBug = true;
        return;
      } else if (savedHasAlphaBug === 'false') {
        self.hasAlphaBug = false;
        return;
      }
      ...
      setSavedAlphaBugResult(self.hasAlphaBug);
    };
    ...
    self.onTimeUpdate = function (currentTime) {
      if (!self._seeking) {
        self.setCurrentTime(currentTime + self.timeOffset);
      }
    };
    ...
    self.resize = function (width, height, top, left) {
      ...
      if (self.canvas.width !== width || self.canvas.height !== height || self.canvas.style.top !== top || self.canvas.style.left !== left) {
        ...
        if (videoSize != null) {
          ...
          if (videoSize.height) {
            self.canvas.style.aspectRatio = videoSize.width / videoSize.height;
          }
  ...
  var _default = _exports.default = SubtitlesOctopus;
});
