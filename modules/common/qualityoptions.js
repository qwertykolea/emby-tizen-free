define(["exports", "./globalize.js"], function (_exports, _globalize) {
  ...
  function getVideoQualityOptions(options) {
    ...
    //const maxAllowedHeight = videoHeight || 2304;

    var qualityOptions = [];

    // have low thresholds to account for odd resolutions, e.g. 1444 * 1080

    if (maxAllowedWidth >= 3400) {
      qualityOptions.push({
        name: '4K - 200 Mbps',
        maxHeight: 2160,
        bitrate: 200000000
    ...
    for (var _i = 0, _length = qualityOptions.length; _i < _length; _i++) {
      var _option = qualityOptions[_i];
      if (_option.maxHeight) {
        _option.maxWidth = Math.ceil(_option.maxHeight * 16 / 9);
      }
    }
