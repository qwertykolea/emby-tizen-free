define(["exports", "./../emby-apiclient/events.js", "./../emby-apiclient/connectionmanager.js", "./servicelocator.js", "./globalize.js"], function (_exports, _events, _connectionmanager, _servicelocator, _globalize) {
  ...
  function loadStrings(plugin) {
    ...
    return _globalize.default.loadStrings({
  ...
  PluginManager.prototype.loadPluginFromUrl = function (url) {
    console.log('Loading plugin: ' + url);
    var instance = this;
    return require([url]).then(function (responses) {
      var pluginFactory = responses[0];
      return instance.loadPlugin(pluginFactory, url);
    });
  };
  PluginManager.prototype.loadPlugin = function (pluginFactory, url) {
    var instance = this;
    return Emby.importModule('./modules/approuter.js').then(function (appRouter) {
      console.log('creating plugin instance from: ' + url);
      ...
      if (!url.includes('://') && !url.startsWith(baseUrl)) {
        url = baseUrl + '/' + url;
      }
      ...
      console.log('loading plugin strings: ' + plugin.id);
      return loadStrings(plugin);
    });
  };
  ...
  PluginManager.prototype.register = function (obj) {
    ...
    _events.default.trigger(this, 'registered', [obj]);
  };
  ...
  var allowedPluginConfigs = ['de228f12-e43e-4bd9-9fc0-2830819c3b92',
  // auto boxsets
  ...
  // Sonos
  '77C172B0-1DCE-443E-9F17-02C74E933B5F',
  // EmbyGuideData
  'C8165D38-EC3D-46E8-B9EB-74FE79DFBADB',
  // Emby.XmlTV
  '4FC3243F-31FE-45F2-A617-DECEEB51B383',
  // SchedulesDirect
  '4F077DD4-7D89-4AF2-B23D-0C36BDA780B2',
  // last.fm
  'de7fe7f0-b048-439e-a431-b1a7e99c930d',
  // EmuMovies
  '076204A5-8820-4776-95C4-5F585C41AC12',
  // Discogs
  'E2F078F2-144A-4B3D-ADC0-65F47945455E',
  // M3U Tuner
  '7CAF8BFD-5B06-4DAE-97C9-11BEDB1C013E',
  // kodi companion
  'b0daa30f-2e09-4083-a6ce-459d9fecdd80',
  // webstreams
  '47A1C3B3-700C-49D6-B75F-7A55D10FC67F'].map(function (i) {
    return i.toLowerCase();
  });
  PluginManager.prototype.allowPluginPages = function (pluginId) {
    return !_servicelocator.appHost.supports('restrictedplugins') || allowedPluginConfigs.indexOf((pluginId || '').toLowerCase()) !== -1;
  };
  PluginManager.prototype.getConfigurationPageUrl = function (name) {
    if (name.toLowerCase().startsWith("configurationpage?name=")) {
      return name;
    }
  ...
  PluginManager.prototype.getConfigurationResourceUrl = function (name) {
    if (_servicelocator.appHost.supports('multiserver')) {
      if (name.toLowerCase().startsWith("configurationpage?name=")) {
        return _connectionmanager.default.currentApiClient().getUrl('web/' + name);
      }
      return _connectionmanager.default.currentApiClient().getUrl('web/ConfigurationPage', {
        name: name
      });
    } else {
  ...
  function loadPluginTranslations(apiClient, pluginId, languages) {
    ...
    var translations = (languages || []).map(function (i) {
      return {
        lang: i,
        path: apiClient.getUrl("web/strings", {
          PluginId: pluginId,
          Locale: i
        })
      };
  ...
  PluginManager.prototype.loadServerPluginTranslations = function (apiClient, pluginId) {
    if (!apiClient._loadedTranslations) {
      apiClient._loadedTranslations = {};
    }
    if (apiClient._loadedTranslations[pluginId]) {
      return Promise.resolve();
    }
    apiClient._loadedTranslations[pluginId] = true;
    if (!apiClient.isMinServerVersion('4.8.0.57')) {
      return Promise.resolve();
    }
    return apiClient.getJSON(apiClient.getUrl("web/stringset", {
      pluginId: pluginId
    })).then(function (languages) {
      return loadPluginTranslations(apiClient, pluginId, languages);
    });
  };
  function loadTranslationFromConfigPage(apiClient, configPage) {
    ...
    return loadPluginTranslations(apiClient, configPage.PluginId, languages);
  }
  PluginManager.prototype.loadServerPluginPageTranslations = function (apiClient, pageName) {
    if (!apiClient._loadedTranslations) {
      apiClient._loadedTranslations = {};
    }
    if (apiClient._loadedTranslations[pageName]) {
      return Promise.resolve();
    }
    apiClient._loadedTranslations[pageName] = true;
    return apiClient.getJSON(apiClient.getUrl("web/configurationpages", {
      Name: pageName
    })).then(function (configPages) {
      if (configPages.length) {
        return loadTranslationFromConfigPage(apiClient, configPages[0]);
      }
    });
  };
  var _default = _exports.default = new PluginManager();
});
