define(["exports", "./idbcore.js"], function (_exports, _idbcore) {
  ...
  function getLibrarytemTypes(serverId) {
    return getLibraryDb(serverId).then(function (db) {
      return _idbcore.default.getDistinctndexKeys(db, 'Index_ItemType');
    });
  }
  function getLibraryItemsByIds(serverId, ids) {
    return getLibraryDb(serverId).then(function (db) {
      return _idbcore.default.getObjectsByKeys(db, ids).then(function (items) {
        items = items || [];
        ...
        items.sort(function (a, b) {
          ...
          return indexedDB.cmp(indexA, indexB);
        });
  ...
  function getLibraryItemsBySyncStatus(serverId, syncStati) {
    return getLibraryDb(serverId).then(function (db) {
      return _idbcore.default.getObjectsByIndexKeys(db, 'Index_SyncStatus', syncStati);
    });
  }
  function getLibraryItemsByType(serverId, itemTypes, options) {
    return getLibraryDb(serverId).then(function (db) {
      return _idbcore.default.getObjectsByIndexKeys(db, 'Index_ItemType', itemTypes);
    });
  }
  function getAllLibraryItems(serverId) {
    return getLibraryDb(serverId).then(function (db) {
      return _idbcore.default.getAllObjects(db);
    });
  }
  ...
  function updateLibraryItem(serverId, key, val) {
    return getLibraryDb(serverId).then(function (db) {
      return _idbcore.default.updateObject(db, key, val);
    });
  }
  ...
  function getLibraryItems(serverId, options) {
    ...
    if (searchParentId) {
      getPromise = getLibraryItem(serverId, searchParentId).then(function (item) {
        if (item && item.Item && item.Item.Type === 'MusicAlbum') {
  ...
  var _default = _exports.default = {
    getLibarytemTypes: getLibrarytemTypes,
