define(["exports", "./../emby-apiclient/connectionmanager.js", "./../emby-apiclient/events.js", "./../common/servicelocator.js"], function (_exports, _connectionmanager, _events, _servicelocator) {
  ...
  var credentials = "(?:(?:[a-z0-9\\$\\-\\_\\.\\+\\!\\*\\'\\(\\)\\,\\;\\?\\&\\=]|(?:\\%[a-f0-9]{2})){1,64}" +
  // username (1-64 normal or url escaped characters)
  "(?:\\:(?:[a-z0-9\\$\\-\\_\\.\\+\\!\\*\\'\\(\\)\\,\\;\\?\\&\\=]|(?:\\%[a-f0-9]{2})){1,25})?" +
  // followed by optional password (: + 1-25 normal or url escaped characters)
  "\\@)";
  ...
  var ipv4 = "(?:25[0-5]|2[0-4][0-9]|[0-1][0-9]{2}|[1-9][0-9]|[1-9])\\." + "(?:25[0-5]|2[0-4][0-9]|[0-1][0-9]{2}|[1-9][0-9]|[1-9]|0)\\." + "(?:25[0-5]|2[0-4][0-9]|[0-1][0-9]{2}|[1-9][0-9]|[1-9]|0)\\." + "(?:25[0-5]|2[0-4][0-9]|[0-1][0-9]{2}|[1-9][0-9]|[0-9])";
  ...
  function getServerAddress(apiClient) {
    ...
    if (!isLocalIpAddress(serverAddress)) {
      // if it's an ip address that isn't localhost, allow it
      if (isValidIpAddress(serverAddress)) {
        return Promise.resolve(serverAddress);
      }

      // if it's ssl, allow it
      if (serverAddress.toLowerCase().startsWith('https:') && _servicelocator.appHost.supports('multiserver')) {
        return Promise.resolve(serverAddress);
      }
    }
  ...
  function getCachedValue(key) {
    var obj = cache[key];
    if (obj && Date.now() - obj.time < 180000) {
      return obj.value;
    }
    return null;
  }
  _events.default.on(_connectionmanager.default, 'localusersignedin', clearCache);
  _events.default.on(_connectionmanager.default, 'localusersignedout', clearCache);
